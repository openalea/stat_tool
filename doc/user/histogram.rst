...
.. _histogram_syntax: syntax.html#type-histogram
...
.. todo:: 
    ...
    Replace  :doc:`Syntax <syntax>` by  :doc:`Syntax <syntax.html#type-histogram>`

Histogram like most of the objects available can be generated by loading an
ASCII file -- following the syntax given in :doc:`Syntax <syntax>` -- or with a list of
values, that will be processed into an histogram.

For instance, let us suppose that you have a valid ASCII file, then simply
load it using the :py:class:`openalea.stat_tool.histogram.Histogram` class so as
...
.. note:: Note here that you can call the methods in two different ways.
    ...
    We advise you to use his function instead of the methods *.display*. The function *Display* will
